/**
 * JavaScript version of Real-Time Quorum & Topology Validation Engine.
 */

export function validateTopologyQuorum(zones, nodes, settings = { maxSkew: 1 }) {
  const errors = [];
  const warnings = [];

  const zoneIds = zones.map(z => z.id);
  const zoneValidatorCounts = {};
  ...
  if (countsArray.length > 0) {
    ...
    const maxAllowedSkew = settings && settings.maxSkew !== undefined ? settings.maxSkew : 1;
